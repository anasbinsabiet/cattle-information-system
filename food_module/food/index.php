    ...
    <div class="card">
      ...
      <div class="card-body">
        <table id="example1" class="table table-bordered table-striped">
          ...
          <tbody>
          <?php
          $query = $connect->prepare("SELECT * FROM `food` WHERE delete_status = 0");
          $query->execute();
          ...
          foreach ($result as $row) {                         
            ...
            <tr>
              ...
              <td><img src="<?php echo $row['food_pic']; ?>" width='70px'></td>
              <td><?php echo htmlspecialchars($row['food_name']) ?></td>
              <td><?php echo get_food_type_name($connect, $row['food_type']) ?></td>
              <td><?php echo htmlspecialchars($row['food_desc']) ?></td>
              <td><?php echo get_food_unit_name($connect, $row['unit']) ?></td>
              <td><?php echo htmlspecialchars($row['unit_price']) ?></td>
              <td><a href="add.php?id=<?php echo((((($row['food_id'] + 9954) * 777) / 100) * 2) * 1971); ?>">
                  <button class="badge badge-success btn-sm">Edit</button>
                </a>
                  <button type="button" name="delete" id="<?php echo $row["food_id"] ?>"
            ...
            $(document).on('click', '.delete', function () {
                ...
                if (confirm("Are you sure you want to Delete?")) {
                    $.ajax({
                        url: "action.php",
                        method: "POST",
                        data: {food_id: food_id, status: status, btn_action: btn_action},
