...
<div class="row mb-2">
  <div class="col-sm-6">
    <h1 class="m-0 text-dark">Food Type</h1>
  </div><!-- /.col -->
  <div class="col-sm-6">
  ...
  <div class="container-fluid">
    <div class="card">
      <div class="card-header">
        <div class="row">
          <div class="col-lg-11">
            <h3 class="card-title">Food Type List</h3>
          </div>
          <div class="col-lg-1">
            <a href="add.php">
      ...
      <div class="card-body">
        <table id="example1" class="table table-bordered table-striped">
          ...
          <tbody>
          <?php
          $query = $connect->prepare("SELECT * FROM `food_type` WHERE delete_status = 0");
          $query->execute();
          $result = $query->fetchAll();
          foreach ($result as $row) {                         
            ?>
            <tr>
              <td><?php echo $row['food_type_id']; ?></td>
              <td><?php echo htmlspecialchars($row['food_type_name']) ?></td>
              <td><a href="add.php?id=<?php echo((((($row['food_type_id'] + 9954) * 777) / 100) * 2) * 1971); ?>">
                  <button class="badge badge-success btn-sm">Edit</button>
                </a>
                  <button type="button" name="delete" id="<?php echo $row["food_type_id"] ?>"
            ...
            $(document).on('click', '.delete', function () {
                ...
                if (confirm("Are you sure you want to Delete?")) {
                    $.ajax({
                        url: "action.php",
                        method: "POST",
                        data: {food_type_id: food_type_id, status: status, btn_action: btn_action},
