    ...
    <div class="card">
      ...
      <div class="card-body">
        <table id="example1" class="table table-bordered table-striped">
          ...
          <tbody>
          <?php
          $query = $connect->prepare("SELECT * FROM `task` WHERE delete_status = 0");
          $query->execute();
          ...
          foreach ($result as $row) {                         
            ...
            <tr>
              ...
              <td><?php if(!empty($row['deadline_date'])){ echo date("d-F-Y", strtotime($row['deadline_date'])); } ?></td>
              <td><?php echo date('g:i A', strtotime($row['dealine_time'])); ?></td>
              <td><a href="add.php?id=<?php echo((((($row['task_id'] + 9954) * 777) / 100) * 2) * 1971); ?>">
                  <button class="badge badge-success btn-sm">Edit</button>
                </a>
                  <button type="button" name="delete" id="<?php echo $row["task_id"] ?>"
            ...
            $(document).on('click', '.delete', function () {
                ...
                if (confirm("Are you sure you want to Delete?")) {
                    $.ajax({
                        url: "action.php",
                        method: "POST",
                        data: {task_id: task_id, status: status, btn_action: btn_action},
