    ...
    <div class="card">
      ...
      <div class="card-body">
        <table id="example1" class="table table-bordered table-striped">
          ...
          <tbody>
          <?php
          $query = $connect->prepare("SELECT * FROM `medicine` WHERE delete_status = 0");
          $query->execute();
          ...
          foreach ($result as $row) {                         
            ...
            <tr>
              ...
              <td><?php echo htmlspecialchars($row['med_gen_name']) ?></td>
              <td><?php echo htmlspecialchars($row['med_purpose']) ?></td>
              <td><?php echo get_medicine_unit_namme($connect, $row['med_unit']); ?></td>
              <td><?php echo htmlspecialchars($row['unit_price']) ?></td>
              <td><a href="add.php?id=<?php echo((((($row['med_id'] + 9954) * 777) / 100) * 2) * 1971); ?>">
                  <button class="badge badge-success btn-sm">Edit</button>
                </a>
                  <button type="button" name="delete" id="<?php echo $row["med_id"] ?>"
            ...
            $(document).on('click', '.delete', function () {
                ...
                if (confirm("Are you sure you want to Delete?")) {
                    $.ajax({
                        url: "action.php",
                        method: "POST",
                        data: {med_id: med_id, status: status, btn_action: btn_action},
